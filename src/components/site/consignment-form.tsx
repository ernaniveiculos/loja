...
export function ConsignmentForm() {
  ...
  function onSubmit(valores: MensagemInput) {
    startTransition(async () => {
      ...
      formData.set("mensagem", `[Consignação] ${valores.mensagem}`);
      const resultado = await enviarMensagem(null, formData);
      if (resultado.erro) {
        toast.error(resultado.erro);
        return;
      }
      toast.success("Solicitação enviada! Entraremos em contato para avaliar seu veículo.");
      form.reset();
    });
  }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField control={form.control} name="mensagem" render={({ field }) => (
          <FormItem>
            <FormLabel>Sobre o veículo</FormLabel>
            <FormControl>
              <Textarea rows={4} placeholder="Marca, modelo, ano, quilometragem e estado de conservação" {...field} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )} />
